...
namespace AdventOfCode
{
    public class DECO2
    {
        static bool IsValidCouple(int left, int right, int sign, out bool changedSign)
        {
            ...
            else if(!(1 <= Math.Abs(diff) && Math.Abs(diff) <= 3))
            {
                ret = false;
            }
            else
        ...
        static bool IsValidCouple(int left, int right)
        {
            int diff = left - right;
            bool ret = true;
        ...
        static bool IsAscending(List<int> levels)
        {
            ...
            for (var i = 0; i < levels.Count() - 1; i++)
            {
                ...
                isDescending = isDescending && (levels[i] > levels[i + 1]);

                // true if this is less than the next and all others so far have been true
                isAscending = isAscending && (levels[i] < levels[i + 1]);
            }
        ...
        {
            bool ret = true;
            List<int> newlevelsByLine = levels.ToArray().ToList();
            newlevelsByLine.RemoveAt(badLevelPos);
            badLevelPos = GetBadLevel(newlevelsByLine, sign);
            if (badLevelPos >= 0)
            {
                ret = false;
            }
        ...
        public static void AdventOfCodeTris()
        {
...
1 3 6 7 9";
            ...
            foreach (var lr in linesReport)
            {
                ...
                if (0 <= badLevelPos && badLevelPos <= (levelsByLine.Count - 1))
                {
                    if (!IsStillSafe(levelsByLine, badLevelPos, sign))
                    {
                        unsafeReportCount++;
                    }
                    else
                ...
                //else if (badLevelPos == levelsByLine.Count - 1)
                //{
                //    ;
                //}
                else
                ...
                else if (0 < badLevelPos && badLevelPos < linesReport.Count() - 1)
                {
                ...
                else if (badLevelPos == linesReport.Count() - 1)
                {
                    ;
                }
                else
